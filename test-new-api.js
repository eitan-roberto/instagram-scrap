...
async function main() {
  ...
  const descResponse = await callGemini({
    contents: [{
      parts: [
        ...
        { text: 'Describe this fashion photo for AI generation. Include outfit, pose, setting, lighting.' }
      ]
    }]
  }, 'gemini-2.5-flash');
  ...
  // Generate with white frame + hairstyle emphasis
  const genResponse = await callGemini({
    contents: [{
      parts: [
        { text: `Create fashion photo: ${description}. Model has long dark hair as in reference. Add clean white border/frame around image with slightly larger bottom border (Polaroid style). Subject centered in frame.` },
  ...
  if (candidate?.finishReason === 'STOP') {
    ...
    // Crop: 4:5 from top + 4% from each side
    ...
    console.log(`✅ Cropped: ${newWidth}x${newHeight} (4:5 + 4% sides)`);
    console.log(`\n📁 Files saved to ${outDir}/`);
  } else {
    console.log('❌ Failed:', candidate?.finishReason || genResponse.error?.message);
  }
}
