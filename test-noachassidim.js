...
const outputDir = './test-noachassidim-batch';

if (!fs.existsSync(outputDir)) fs.mkdirSync(outputDir, { recursive: true });

// Read CSV and pick random images
const csvPath = './data/noachassidim-scraped.csv';
...
for (const line of lines) {
  ...
  // Extract shortcode from URL (e.g., /p/DUdpoCMDsuY/)
  const url = parts[0];
  const shortcodeMatch = url.match(/\/p\/([^\/]+)/);
  const shortcode = shortcodeMatch ? shortcodeMatch[1] : 'unknown';
  
  const imageUrls = parts[4]?.split('|') || [];
  
  for (const imgUrl of imageUrls) {
    if (imgUrl && imgUrl.includes('instagram.com') && !imgUrl.includes('giphy')) {
      allImages.push({ shortcode, url: imgUrl.trim() });
    }
  }
}
...
console.log('🎨 NOACHASSIDIM BATCH: Describe → Generate\n');
